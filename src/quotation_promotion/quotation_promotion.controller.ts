...
export class QuotationPromotionController {
  ...
  async create(@Body() createQuotationPromotionDto: CreateQuotationPromotionDto, @UploadedFiles() quotation_promotion_evidences: Express.Multer.File[], @Req() req: RequestWithUser) {
    return await this.quotationPromotionService.create(createQuotationPromotionDto, quotation_promotion_evidences, req.user);
  }
  ...
  @Post(':id')
  @UseInterceptors(FilesInterceptor('quotation_promotion_evidences'))
  async update(@Param('id') id: string, @Body() updateQuotationPromotionDto: UpdateQuotationPromotionDto, @UploadedFiles() quotation_promotion_evidences: Express.Multer.File[], @Req() req: RequestWithUser) {
    return await this.quotationPromotionService.update(+id, updateQuotationPromotionDto, quotation_promotion_evidences, req.user);
  }
